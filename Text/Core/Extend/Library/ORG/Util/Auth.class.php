...
class Auth{
    ...
    protected $_config = array(
		...
		'ADMINISTRATOR'=> array('10001'),//超级管理员
        'AUTH_GROUP' => 'auth_group', //用户组数据表名
        'AUTH_GROUP_ACCESS' => 'auth_group_access', //用户组明细表
        'AUTH_RULE' => 'auth_rule', //权限规则表
        'AUTH_USER' => 'lty_account'//用户信息表
    );
	...
	public function isSuper($uid){
		return in_array($uid,$this->_config['ADMINISTRATOR']);
	}
    ...
    public function check($name, $uid, $relation='or') {
		...
		if ($this->isSuper($uid)) return true;

        $authList = $this->getAuthList($uid);
        foreach ($authList as $val) {
            if($val==$name) return true;
            if(strpos($val, '*')!==false){
                if(substr($val, 0,strpos($val, '-'))==substr($name, 0,strpos($name, '-'))) return true;
            }
        }
        return false;
    }
    ...
    protected function getUserInfo($uid) {
        ...
        if(!isset($userinfo[$uid])){
             $userinfo[$uid]=M()->table($this->_config['AUTH_USER'])->where(array('uid'=>$uid))->find();
        }
        return $userinfo[$uid];
    }
