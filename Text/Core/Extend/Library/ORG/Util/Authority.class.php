...
class Authority {
    ...
    protected $_config = array(
        ...
        'AUTH_GROUP' => 'gfong_auth_group', //用户组数据表名
        'AUTH_GROUP_ACCESS' => 'gfong_auth_group_access', //用户组明细表
        'AUTH_RULE' => 'gfong_auth_rule', //权限规则表
        'AUTH_USER' => 'gfong_user'//用户信息表
    );
    ...
    public function getGroups($uid) {
        ...
        if (!empty($groups[$uid]))
            return $groups[$uid];
        $groups[$uid] = 
        M()->table($this->_config['AUTH_GROUP_ACCESS'] . ' a')
            ->where("a.uid='$uid'")
    ...
    protected function getUserInfo($uid) {
        ...
        if(!isset($userinfo[$uid])){
            $where['uid'] = $uid;
            $userinfo[$uid]=M()->table($this->_config['AUTH_USER'])->where($where)->find();
        }
        return $userinfo[$uid];
    }
