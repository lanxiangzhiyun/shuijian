...
<script type="text/javascript" src="/newadmin/Public/civi/js/jquery.min.js"></script>
<script>
...
function stateyes(id,val){
	...
	var rr=Number(data.listid)+1;
		...
		if(data.status==1){
			$("#t1").show(100);
			$("#t1").html('恭喜，回答正确').delay(500).fadeOut();
			$("#p"+data.listid).delay().fadeOut();
			$("#r"+data.listid).delay().fadeOut();
			$("#p"+rr).delay(500).fadeIn();
			$("#r"+rr).delay(500).fadeIn();
    ...
    <script language="javascript" type="text/javascript">  
		...
		//关闭页面操作
            $(window).unload(function() {
                ...
                $.post(<?php echo U("addtime");?>, {addtime:state, aid:<?php echo ($_SESSION['aid']); ?>);  
            });  
        });  
    </script> 
