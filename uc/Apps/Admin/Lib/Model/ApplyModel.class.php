...
class ApplyModel extends Model {
	// 数据库表
	protected $trueTableName = 'boqii_apply';
	...
	public function getApplyList($param){
		...
		$applyList = $this->where($where)->order('id DESC')->limit("$pageStart, $pageNum")->select();
		
		// 当前页条数
		$this->subtotal = count($applyList);
		//总页数
		...
		foreach($applyList as $key=>$val){
			...
			switch ($val['type']) {
				...
				case '3':
					...
					$applyList[$key]['age'] 	= '-';
					$applyList[$key]['weibo']	= '-';
					$applyList[$key]['weixin'] 	= '-';
					$applyList[$key]['pet_num'] = '-';
					$applyList[$key]['people_num'] = '-';
					break;
				case '4':
					$applyList[$key]['mobile'] 	= '-';
					$applyList[$key]['qq'] 		= '-';
					$applyList[$key]['weibo']	= '-';
					$applyList[$key]['weixin'] 	= '-';
					$applyList[$key]['pet_type'] = '-';
					$applyList[$key]['pet_num'] = '-';
					$applyList[$key]['people_num'] = '-';
					break;
				case '5':
					$applyList[$key]['sex'] 	= $val['sex'] == 1 ? '大型' : ($val['sex'] == 2 ? '小型' : '') ;
					$applyList[$key]['age'] 	= $val['age'] == 1 ? '免费接送' : ($val['sex'] == 2 ? '自驾' : '') ;
					$applyList[$key]['qq'] 		= '-';
					$applyList[$key]['weibo'] 	= '-';
					$applyList[$key]['weixin'] 	= '-';
					$applyList[$key]['city_name'] = '-';
					if ($val['pet_type'] == 1) {
						$applyList[$key]['pet_type'] = '5月23日（虹泰环城东路总院/建国分院）';
					}elseif ($val['pet_type'] == 2) {
						$applyList[$key]['pet_type'] = '5月24日（虹泰滨江彩虹分院）' ;
					}elseif ($val['pet_type'] == 3) {
						$applyList[$key]['pet_type'] = '5月30日（虹泰萧山分院/月明路分院）' ;
					}elseif ($val['pet_type'] == 4) {
						$applyList[$key]['pet_type'] = '5月31日（虹泰湖墅分院）' ;
					}elseif ($val['pet_type'] == 5) {
						$applyList[$key]['pet_type'] = '6月6日（虹泰专场）' ;
					}elseif ($val['pet_type'] == 6) {
						$applyList[$key]['pet_type'] = '6月7日（虹泰专场）' ;
					}elseif ($val['pet_type'] == 7) {
						$applyList[$key]['pet_type'] = '6月13日' ;
					}elseif ($val['pet_type'] == 8) {
						$applyList[$key]['pet_type'] = '6月14日' ;
					}else{
					 	$applyList[$key]['pet_type'] = '' ;
					}
					break;
