 ...
 * @created: 13-1-16
 */
class BkCategoryModel extends Model {
	protected $trueTableName = 'bk_category';
	...
	public function getAllTopList ($params,$fields = '') {
		...
		//本页条数
		$this->count = $this->where(array('status' => array('egt', 0), 'level' => $params['level']))->limit($page_start . ',' . $page_num)->count();
		//总条数
		$total = $this->where(array('status' => array('egt', 0), 'level' => $params['level']))->count();
		//总页数
		$this->totalPage = ceil($total / $page_num);
		$arrAllTopList = $this->where(array('status' => array('egt', 0), 'level' => $params['level']))
			->field($fields)->order('create_time desc ')->limit($page_start . ',' . $page_num)->select();
	...
	public function getTeamPkId($id) {
	   $intTid = M('bk_team')->where(array('category_id'=>$id))->getField('id');
		return $intTid;
	}
