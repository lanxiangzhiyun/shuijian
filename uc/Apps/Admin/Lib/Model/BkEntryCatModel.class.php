 ...
 * @author Fongson
 */
class BkEntryCatModel extends Model {
	// 数据库表
	protected $trueTableName = 'bk_entry_cat';
	...
	public function getAllTopList ($params, $fields = '') {
		...
		foreach($arrAllTopList as $key=>$val) {
			$arrAllTopList[$key]['parent_name'] = M()->Table('bk_entry_cat')->where('id='.$val['parent_id'])->getField('name');
		}
	 ...
	 * @return array
	 */
	public function  getBkEntryCat ($id) {
		$strSubList = '<option value="0" selected>请选择</option>';
		//分类id 和 父id
		$fields = 'id,parent_id,level';
		$arrInfo = $this->getInfo($id, $fields);
