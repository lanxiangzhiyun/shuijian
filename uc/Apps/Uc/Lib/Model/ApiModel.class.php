...
class ApiModel {
	...
	private function _getUserDetailInfo($uid) {
		...
		$extend = M('boqii_users_extend')->where('uid='.$uid)->field('bday,qq,gender,avatar,sightml,carrer,lovepet,interested,detailaddress,totalsign,guide,city_id,password_strength,pay_password_strength')->find();
		if($extend){
		...
		if($userinfo['is_baike']==1){	//百科用户相关信息
			$baikeUser = M()->Table('boqii_users_extendbaike')->where(array('uid'=>$uid))->field('uid,name,pic_path,level,introduce,attention_num,flower_num')->find();
