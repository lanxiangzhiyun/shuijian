BQ.add('BQAjaxUP', function(W,CLASS_NAME){
...
// 筛选后自动提交文件
autoSubmit: true,
// 您希望从服务器返回的数据类型HTML (text) 和 XML 自动检测；当您使用JSON作为响应时很实用，在这种情况下就设置为 “json”；也要设置服务器端的响应类型为text/html, 否则在IE6下是不工作的
responseType: false,
// 文件选择后使用，当autoSubmit为disabled时很管用，您可以通过返回false取消上传
// @参数file为上传文件的文件名
// @参数extension为那个文件（后缀名）
onChange: function(file, extension){},
// 文件上传时调用，您可以通过设置返回false 取消上传
// @参数file为上传文件的文件名
// @参数extension为那个文件（后缀名）
onSubmit: function(file, extension) {},
// 当文件上传完成的时候调用，警告！不要使用”false”字符串作为服务器的响应
// @参数file 指上传的文件名
// @参数 response 指服务器的响应
onComplete: function(file, response) {}
});
