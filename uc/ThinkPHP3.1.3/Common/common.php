...
/**
* 静态文件更新后自动刷新(加参数-文件修改时间)
* @path 静态资源相对/static的目录
*/
function autoVer($path){
	$staticUrl = C('STATIC_DIR');
	$staticDirPath = C('STATIC_DIR_PATH');
	$ver = filemtime($staticDirPath.$path);
	return $staticUrl.$path.'?v='.$ver;
}

/**
* api日志记录
* @param apiName api名称
* @param type 如果参数用param,如果是返回结果result
* @param data 参数或结果(不要做序列化处理，接口里面统一处理)
*/
function apiLog($apiName,$type,$data){
	$str = date('Y-m-d H:i:s')."\t".$apiName."\t".$type."\t".serialize($data)."\r\n";
	error_log($str,3,C('API_LOG_VET_DIR').'vet_'.date('Ymd').'.log');
}